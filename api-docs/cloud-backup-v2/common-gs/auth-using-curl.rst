...
requests to the API. For more information, see
:ref:`Configure environment variables<configure-environment-variables>`.
...
token ID
    ...
    expire, unless it is revoked prior to the
    expiration. To get a new token, submit another authentication request. For
...
endpoints
	  The endpoints value provides the URL that you use to access the API
    service. For guidance on choosing an endpoint, see
...
request, prefix the variable name with a $ symbol, for example,
``$API_ENDPOINT``.
...
#. Export the tenant ID to an environment variable
   ...
   tenant name. Replace ``tenant-id`` in the following example with the tenant
   ``id`` value returned in the authentication response.
...
#. In the ``service catalog`` section of the authentication response, copy the
   ...
   .. note::
       ...
       (account ID). With Cloud Backup v2.0, you do not have to use the tenant
       ID. See :ref:`Service access endpoints <service-access-endpoints>`.

#. Export the URL to an environment variable, as shown in the following
   example. Replace ``publicURL`` in the example with the publicURL value

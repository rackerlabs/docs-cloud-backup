...
token ID
    The authentication token ID value is required to confirm your identity each time you access the service.
    You include it in the ``X-Auth-Token`` header for each API request.
...
#. In the ``token`` section of the authentication response, copy the token ``id`` and
   tenant ``id`` values from the token object. The following example shows example values only.
